function [U,x,t] = AdvectionDiffusion(fun,N,M,tmax,epsilon,mesh,a)

if strcmp(mesh,"uniform") % Uniform mesh
...
%     end
    ...
    vec = fdcoeffF(2, x(2), x((1):(3))).*ones(m,1);
    FTCS2 = spdiags(vec,-1:1,m,m);
    FTCS2 = FTCS2(2:(end-1),:);
    
    % First derivative of the spatial variable
    vec = fdcoeffF(1, x(2), x((1):(2))).*ones(m,1);
...
%     vec = fdcoeffF(2, x(2), x((1):(3))).*ones(m1+1,1);
%     temp = spdiags(vec,-1:1,m1+1,m1+1);
%     FTCS2(1:(m1-1),1:(m1+1)) = temp(2:(end-1),:);
%     FTCS2((m1+m2):end,(m1+m2):end) = temp(2:(end-1),:);
% 
%     vec = fdcoeffF(2, x2(2), x2((1):(3))).*ones(m2,1);
%     temp = spdiags(vec,-1:1,m2,m2);
%     temp = temp(2:(end-1),:);
%     FTCS2((m1+1):(m1+m2-2),(m1+1):(m1+m2)) = temp;
% 
%     FTCS2(m1,m1:(m1+2)) = fdcoeffF(2, x2(1), [x1(end) x2(1) x2(2)]);
%     FTCS2(m1+m2-1,(m1+m2-1):(m1+m2+1)) = fdcoeffF(2, x2(end), [x2(end-1) x2(end) x3(1)]);
    
    % First derivative of the spatial variable
    %FTBS = zeros(m-2,m);
% 
%     vec = fdcoeffF(1, x1(2), x1(1:2)).*ones(m1+2,1);
%     temp = spdiags(vec,-1:0,m1+2,m1+2);
%     temp = temp(2:(end-1),:);
%     FTBS(1:m1,1:(m1+2)) = temp;
%     FTBS((end-m3+2):end,(end-m3):end) = temp(2:end,2:end);
% 
%     vec = fdcoeffF(1, x2(2), x2(1:2)).*ones(m2,1);
...
%temp = fdcoeffF(2, x(2), x(1:3));
%A(1,1:2) = temp(2:3);
for i=2:(m-1)
    FTCS2(i,i-1:i+1) = fdcoeffF(2, x(i), x((i-1):(i+1)));
end
%temp = fdcoeffF(2, x(m-1), x((m-2):m));
%A(m,(m-1):m) = temp(1:2);
